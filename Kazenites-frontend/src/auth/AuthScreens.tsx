...
export function RegisterScreen() {
  ...
  const onSubmit = async () => {
    ...
    setLoading(true);
    try {
      await register(email.trim(), password, name.trim(), city.trim() || undefined);
    } catch (e: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput placeholder="City (optional)" value={city} onChangeText={setCity} style={styles.input} />
      <TextInput placeholder="Email" autoCapitalize="none" value={email} onChangeText={setEmail} style={styles.input} />
      <TextInput placeholder="Password" secureTextEntry value={password} onChangeText={setPassword} style={styles.input} />
      {error && <Text style={styles.error}>{error}</Text>}
